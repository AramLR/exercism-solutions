...
class Tournament
  ...
  def self.tally(matches)
    ...
    matches.each_line do |line|
      ...
      host, visitor, result = line.chomp.split(';')
      tally_results.add_team host
      tally_results.add_team visitor

      case result
      when 'win'
        tally_results.victory_for! host
        tally_results.defeat_for! visitor
      when 'draw'
        tally_results.draw_for! host
        tally_results.draw_for! visitor
      when 'loss'
        tally_results.defeat_for! host
        tally_results.victory_for! visitor
      else
