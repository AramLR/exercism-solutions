...
class Anagram
  ...
  def match(candidates)
    candidates.select do |word|
      @initial_word.downcase.chars.all? {|char| word.downcase.count(char) == @initial_word.downcase.count(char)}
      && @initial_word.downcase != word.downcase
      && @initial_word.size == word.size
    end
  end
end
